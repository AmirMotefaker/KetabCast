# 📖 KetabCast (کتاب‌کست) — Master Product & Engineering Document
...
**کتاب‌کست** یک پلتفرم فارسی‌زبان Open Source است که خلاصه‌ی کتاب‌های ارزشمند (غیرداستانی در اولویت اول: کسب‌وکار، روان‌شناسی، رشد فردی، تاریخ) را به شکل **اپیزودهای صوتی کوتاه فارسی (۱۰–۱۸ دقیقه)** با کیفیت استودیویی و روایت جذاب تبدیل می‌کند؛ با کمک یک پایپ‌لاین هوش مصنوعی end-to-end (متن → خلاصه → اسکریپت صوتی → TTS فارسی → پست‌پروداکشن).

وضعیت فعلی ریپو (`main` branch):
- **v0.1.0 – Foundation** ✅ انجام شده: یک وب‌اپ Next.js (App Router, TS) با تم تاریک بنفش، RTL، فونت وزیرمتن، صفحه هر کتاب با ایده‌های کلیدی و ترنسکریپت، پلیر اختصاصی، Static Export روی GitHub Pages.
- محتوای صوتی واقعی هنوز وجود ندارد (Placeholder).
- بک‌اند، دیتابیس، پایپ‌لاین AI، سیستم کاربری، پرداخت — هیچ‌کدام پیاده نشده‌اند.
...
**چشم‌انداز:** تبدیل کتاب‌کست به Blinkist/Headway فارسی‌زبان منطقه (ایران + جامعه فارسی‌زبان افغانستان/تاجیکستان/دیاسپورا)، جایی که یادگیری از کتاب در حین رانندگی، ورزش یا کار روزمره اتفاق می‌افتد.
...
| | Blinkist/Headway | نوار/کتابراه (فارسی) | **کتاب‌کست** |
|---|---|---|---|
...
- [ ] **تکمیل پلیر در v0.2**: سرعت پخش، Progress/Seek و دانلود/آفلاین
...
- [ ] تکمیل پلیر صوتی: Progress/Seek، سرعت پخش و دانلود/آفلاین
